...
export const AuthProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  /**
   * Fetch user profile via Cloud Function (no direct RTDB access).
   */
  const fetchUserProfile = async (uid: string): Promise<UserProfile> => {
    try {
      const data = await serverApiFetch<{ id: string; [key: string]: any }>(
        `/api/userProfile?uid=${uid}`,
      );

      // The server returns { id: recordId, ...profileData }
  ...
  /**
   * Update lastLogin via Cloud Function (no direct RTDB write).
   */
  const touchLastLogin = async (recordId: string | null) => {
    if (!recordId) return;

    try {
      await serverApiFetch("/api/updateLastLogin", {
        method: "POST",
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      try {
        ...
        if (!prefetchDoneRef.current) {
          ...
          prefetchCommonData(accessible).catch(() => {
