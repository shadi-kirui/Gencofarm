...
import { Download, Warehouse, Eye, Calendar, Building, DollarSign, Package, Archive, Edit, Save, X, Upload, Trash2, Plus, RotateCcw } from "lucide-react";
...
import { readCachedValue, writeCachedValue, cacheKey } from "@/lib/data-cache";
...
interface Stats {
  totalRevenue: number;
  totalBales: number;
  totalPurchasedBales: number;
  totalHarvestedBales: number;
  totalBalesSold: number;
  totalFacilities: number;
  totalBalesBalance: number;
}

// Kenya Areas API types
interface KenyaAreasResponse {
  [county: string]: { [subcounty: string]: string[] };
}
...
const HayStoragePage = () => {
  ...
  const [stats, setStats] = useState<Stats>({
    totalRevenue: 0,
    totalBales: 0,
    totalPurchasedBales: 0,
    totalHarvestedBales: 0,
    totalBalesSold: 0,
    totalFacilities: 0,
    totalBalesBalance: 0,
  });
  ...
  // -- Kenya Areas API for county/subcounty selection --
  const [kenyaAreas, setKenyaAreas] = useState<KenyaAreasResponse | null>(null);

  useEffect(() => {
    const fetchKenyaAreas = async () => {
      try {
        const cached = readCachedValue<KenyaAreasResponse>(KENYA_AREAS_CACHE_KEY, KENYA_AREAS_CACHE_TTL_MS);
        if (cached) { setKenyaAreas(cached); return; }
        const res = await fetch(`${KENYA_AREAS_API_URL}?apiKey=${KENYA_AREAS_API_KEY}`);
        if (!res.ok) return;
        const data = await res.json();
        if (data && typeof data === "object" && !Array.isArray(data)) {
          setKenyaAreas(data);
          writeCachedValue(KENYA_AREAS_CACHE_KEY, data);
        }
      } catch (err) {
        console.error("Failed to load Kenya areas:", err);
      }
    };
  ...
  const countyOptions = useMemo(() => {
    if (!kenyaAreas) return [];
    return Object.keys(kenyaAreas).sort((a, b) => a.localeCompare(b));
  }, [kenyaAreas]);

  const getSubcountyOptions = useCallback((county: string): string[] => {
    if (!kenyaAreas || !county) return [];
    const matchKey = Object.keys(kenyaAreas).find(k => k.toLowerCase() === county.toLowerCase());
    if (!matchKey) return [];
    return Object.keys(kenyaAreas[matchKey]).sort((a, b) => a.localeCompare(b));
  }, [kenyaAreas]);

  const addSubcountyOptions = useMemo(() => getSubcountyOptions(addingRecord.county || ""), [addingRecord.county, getSubcountyOptions]);
  const editSubcountyOptions = useMemo(() => getSubcountyOptions(editingRecord?.county || ""), [editingRecord?.county, getSubcountyOptions]);
  ...
  const applyFilters = useCallback(() => {
    ...
    const totalHarvestedBales = sortedFiltered.filter(r => normalizeBaleSource(r.bale_source) === "harvested").reduce((sum, r) => sum + getStoredBales(r), 0);
    const totalPurchasedBales = sortedFiltered.filter(r => normalizeBaleSource(r.bale_source) === "purchased").reduce((sum, r) => sum + getStoredBales(r), 0);
    const totalBales = totalHarvestedBales + totalPurchasedBales;
    ...
    setStats({
      ...
      totalBalesBalance: totalBales - totalBalesSold
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
        <StatsCard title="Bales" value={`${millify(stats.totalBales)}`} icon={Package} description={
          <>
            <span className="whitespace-nowrap">Purchased: {stats.totalPurchasedBales.toLocaleString()}</span>
            <span className="whitespace-nowrap">Harvested: {stats.totalHarvestedBales.toLocaleString()}</span>
      ...
      <Card className="shadow-lg border-0 bg-white">
        <CardContent className="space-y-4 pt-6">
          <div className="flex items-center justify-between">
            ...
            />
            <Button variant="outline" size="sm" onClick={clearAllFilters} className="shrink-0 ml-2 text-xs">
          ...
          <div className="space-y-6 py-4 overflow-y-auto max-h-[60vh]">
            <div className="bg-slate-50 rounded-xl p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2"><Label htmlFor="add-county" className="text-sm font-medium text-slate-600">County *</Label><Select value={addingRecord.county || ''} onValueChange={(value) => setAddingRecord(prev => ({ ...prev, county: value, subcounty: '' }))}><SelectTrigger className="border-gray-300 focus:border-blue-500"><SelectValue placeholder="Select county" /></SelectTrigger><SelectContent>{countyOptions.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div>
                <div className="space-y-2"><Label htmlFor="add-subcounty" className="text-sm font-medium text-slate-600">Subcounty *</Label><Select value={addingRecord.subcounty || ''} onValueChange={(value) => setAddingRecord(prev => ({ ...prev, subcounty: value }))}><SelectTrigger className="border-gray-300 focus:border-blue-500"><SelectValue placeholder="Select subcounty" /></SelectTrigger><SelectContent>{addSubcountyOptions.map(sc => <SelectItem key={sc} value={sc}>{sc}</SelectItem>)}</SelectContent></Select></div>
            ...
            <div className="space-y-6 py-4 overflow-y-auto max-h-[60vh]">
              <div className="bg-slate-50 rounded-xl p-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2"><Label htmlFor="edit-county" className="text-sm font-medium text-slate-600">County</Label><Select value={editingRecord.county || ''} onValueChange={(value) => { handleEditChange('county', value); handleEditChange('subcounty', ''); }}><SelectTrigger className="border-gray-300 focus:border-blue-500"><SelectValue placeholder="Select county" /></SelectTrigger><SelectContent>{countyOptions.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div>
                  <div className="space-y-2"><Label htmlFor="edit-subcounty" className="text-sm font-medium text-slate-600">Subcounty</Label><Select value={editingRecord.subcounty || ''} onValueChange={(value) => handleEditChange('subcounty', value)}><SelectTrigger className="border-gray-300 focus:border-blue-500"><SelectValue placeholder="Select subcounty" /></SelectTrigger><SelectContent>{editSubcountyOptions.map(sc => <SelectItem key={sc} value={sc}>{sc}</SelectItem>)}</SelectContent></Select></div>
