...
// Kenya Areas API
interface KenyaAreasResponse { [county: string]: { [subcounty: string]: string[] }; }
...
const AnimalHealthPage = () => {
  ...
  useEffect(() => {
    const fetchAreas = async () => {
      ...
      } catch (err) { console.error("Failed to load Kenya areas:", err); }
    };
    void fetchAreas();
  }, []);
  ...
  const formSubcountyOptions = useMemo(() => {
    if (!kenyaAreas || !activityForm.county) return [];
    const matchKey = Object.keys(kenyaAreas).find(k => k.toLowerCase() === activityForm.county.toLowerCase());
    if (!matchKey) return [];
    return Object.keys(kenyaAreas[matchKey]).sort((a, b) => a.localeCompare(b));
  }, [kenyaAreas, activityForm.county]);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setActivityForm({ ...activityForm, county: value, subcounty: "" })}
                    >
                      <SelectTrigger><SelectValue placeholder="Select county" /></SelectTrigger>
                      <SelectContent>{countyOptions.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setActivityForm({ ...activityForm, subcounty: value })}
                    >
                      <SelectTrigger><SelectValue placeholder="Select subcounty" /></SelectTrigger>
                      <SelectContent>{formSubcountyOptions.map(sc => <SelectItem key={sc} value={sc}>{sc}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-2">
                    <Label>
            ...
            <CardContent className="pb-4 px-4">
              ...
              <div className="flex items-center gap-4 mt-2">
                <p className="text-xs text-slate-500 flex items-center">
                  ...
                  Male ({totalBeneficiaries > 0 ? ((totalMaleBeneficiaries / totalBeneficiaries) * 100).toFixed(1) : 0}%)
                </p>
                <p className="text-xs text-slate-500 flex items-center">
                  ...
                  Female ({totalBeneficiaries > 0 ? ((totalFemaleBeneficiaries / totalBeneficiaries) * 100).toFixed(1) : 0}%)
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <DialogContent className="sm:max-w-[900px] bg-white rounded-2xl border-0 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid gap-6 py-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <Label>County</Label>
                  <Select value={activityForm.county} onValueChange={(value) => setActivityForm({ ...activityForm, county: value, subcounty: "" })}>
                    <SelectTrigger><SelectValue placeholder="Select county" /></SelectTrigger>
                    <SelectContent>{countyOptions.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  <Label>Subcounty</Label>
                  <Select value={activityForm.subcounty} onValueChange={(value) => setActivityForm({ ...activityForm, subcounty: value })}>
                    <SelectTrigger><SelectValue placeholder="Select subcounty" /></SelectTrigger>
                    <SelectContent>{formSubcountyOptions.map(sc => <SelectItem key={sc} value={sc}>{sc}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
