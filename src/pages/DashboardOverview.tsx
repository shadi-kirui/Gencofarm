...
import { fetchCollectionsBatch, fetchCollectionByProgrammes } from "@/lib/firebase";
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const pageData = await fetchCollectionsBatch<OverviewRecord>([
        ...
        void (async () => {
          const activities = await fetchCollectionByProgrammes<OverviewRecord>(
            "Recent Activities",
            programmesToRead,
            {
              startDate: currentMonthRange.startDate,
          ...
          <div className="relative flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between lg:gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                variant="ghost"
                size="sm"
                className="h-9 rounded-xl border-white/20 bg-white/12 px-3 text-xs text-white backdrop-blur-sm hover:bg-white/20 hover:text-white sm:h-10"
                onClick={() => setSelectedProgramme(accessibleProgrammes.length > 1 ? "ALL" : accessibleProgrammes[0] || "")}
